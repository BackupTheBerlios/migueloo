...
class miguel_VStatistics extends miguel_VMenu
{
    ...
    function right_block() 
    {
		...
		//Cabecera
		$elem1 = html_td('', '', html_b('Perfil'));
		$elem1->set_tag_attribute('width', '50%');
		$elem2 = html_td('', '', html_b('Conexiones'));
		$elem2->set_tag_attribute('align', 'right');
		$elem3 = html_td('', '', html_b('%'));
		...
		for ($i=0; $i<count($arrUsers); $i++) {
			$elem1 = html_td('', '', $arrUsers[$i]['user.user_alias']);
			$elem2 = html_td('', '', $arrUsers[$i]['countLogin']);			
			$elem2->set_tag_attribute('align', 'right');
		...
		$elem1 = html_td('', '', html_b('TOTAL'));
		$elem2 = html_td('', '', html_b($iTotalCon));			
		$elem2->set_tag_attribute('align', 'right');
		$elem3 = html_td('', '', html_b(100));			
		$elem3->set_tag_attribute('align', 'right');
		$table->add_row($elem1, $elem2, $elem3);
