...
class miguel_MEmail extends base_Model
{
    ...
    function sendMessage($arrId, $str_subject, $str_body)
    {
       ...
       $iMyId = Session::getValue('USERINFO_USER_ID');
        ...
        $iMsgId = $this->Insert('message',
                                 ...
                                 "$iMyId,$str_subject,$str_body,$now");
    ...
    /*===================================================================
    	Devuelve los mensajes recibidos por el usuario actual
    	===================================================================*/
    function getUserMessages(/*$max_level=1*/)
    {
     		...
     		$iMyId = Session::getValue('USERINFO_USER_ID');
				...
				$ret_val = $this->SelectMultiTable('message,receiver_message,user', 
                                                ...
                                                "user.user_id = receiver_message.id_receiver AND receiver_message.id_message = message.id AND receiver_message.id_receiver = $iMyId AND receiver_message.status < 2");
    ...
    function getSendedMessages()
    {
     		...
     		$iMyId = Session::getValue('USERINFO_USER_ID');
				...
				$ret_val = $this->SelectMultiTable('message,receiver_message,user', 
                                                'user.user_alias, message.sender,message.subject,message.body,message.date,receiver_message.status,message.id',
                                                "user.user_id = receiver_message.id_receiver AND receiver_message.id_message = message.id AND message.sender = $iMyId AND receiver_message.status < 2");
