...
class miguel_VDetail extends miguel_VMenu
{
    ...
    function _contactDetail()
    {
        ...
        if ( is_array($contact)) {
            $ul = html_ul();
			$elem = container();
			$elem->add($contact[0]["contact.contact_name"]);
            $elem->add($contact[0]["contact.contact_surname"]);
            $elem->add(html_a(Util::format_URLPath("contact/index.php", "contact_id=".$this->getViewVariable('contact_id')."&option=delete"), "[Eliminar]", null, "_top"));
            $ul->add($elem);
            $div->add($ul);
