...
class miguel_MCalendar extends base_Model
{
	/**
	 * This is the constructor.
	 *
	 */
    function miguel_MCalendar() 
    {
    ...
    {       
        ...
        $ret_val = $this->Insert('calendar',
                                 ...
                                 "$id_course, $event, $subject, $content, $dt_ini, $dt_fin, 0, $dt_aud, $user_aud");
    ...
    function getEvents()
    {
		 $data = $this->Select('calendar', 'calendar_id, course_id, event_type_id, title, description, date_start, date_end, all_day, aud_time, aud_user_id');
    	...
    	if ($this->hasError()) {
    		...
    		for ($i=0; $i < $count; $i++) {
                $ret_val[] = array ( 'calendar' => $data[$i]['calendar.calendar_id'],
									 ...
									 'event_type' => $this->getEventTypeDescription($data[$i]['calendar.event_type_id']),
