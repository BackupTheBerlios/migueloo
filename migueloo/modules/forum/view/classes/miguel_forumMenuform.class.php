...
class miguel_forumMenuform extends base_FormContent 
{
    ...
    function form_init_elements() 
    {
        ...
        //$this->set_confirm();
				...
				$this->add_element($this->_formatElem("FEListBox", "orden", "orden", FALSE, "100px", NULL, $arrOrden));
				
  			//AÃ±ade un boton con la acciÃ³n submit
        $submit = $this->_formatElem('base_SubmitButton', 'Aceptar', 'submit', 'Ordenar');
        $submit->set_attribute('accesskey','e');               
        $this->add_element($submit);    		  
        
				$this->add_hidden_element('status');
    		$this->set_hidden_element_value('status', 'list');
