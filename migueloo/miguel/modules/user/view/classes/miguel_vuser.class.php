...
include_once (Util::app_Path("todo/view/include/classes/miguel_userform.class.php"));

class miguel_VUser extends miguel_VMenu
{
     ...
     *
	 */
    function miguel_VUser($title, $arr_commarea)
    {
    ...
    function right_block() 
    {
		...
		$div = html_div('ul-big');
		$div->add(html_img(Util::app_URLPath('image/menu/inscription.png'), 0, 0, 0, ''));
		$div->add(html_b('Alta de nuevo Usuario'));
		$div->add(html_br(2));
		$ret_val->add($div);
		...
		if($this->getViewVariable('bol_cuestion')){
            $div->add('Rellene el siguiente formulario y pulse validar.');
    		$div->add(html_br(2));
    		$ret_val->add($div);
	        $ret_val->add(new FormProcessor(new miguel_userForm(), 'sugest', Util::format_URLPath('lib/user/index.php')));
        } else {
            ...
            $ret_val->add($table);
            $ret_val->add(new FormProcessor(new miguel_navForm(), 'nav', Util::format_URLPath('/lib/user/index.php')));
        }
