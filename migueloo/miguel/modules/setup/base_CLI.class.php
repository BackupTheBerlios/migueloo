 ...
 *
 * $Horde: framework/CLI/CLI.php,v 1.33 2004/02/04 15:15:31 mdjukic Exp $
 *
 ...
 * @package Horde_CLI
 *
 * Modificado por Jesús Martínez <jamarcer@inicia.es>
 * Cambio de _() por agt() para uso en miguel
 * Cambio de literales.
 *
 * Renombrada clase para evitar confusiones
 */
class base_CLI {
     ...
     *                               'cli.warning', 'cli.success', 
     *                               'cli.message' or 'cli.label'.
     */
    ...
    function fatal($error)
    {
        ...
        $this->writeln();
        $this->writeln($this->red(agt('fatalErr')));
        ...
        while (true) {
            ...
            if (is_array($choices) && !empty($choices)) {
                ...
                if (isset($choices[$response])) {
                    ...
                    $this->writeln(sprintf("%s ", $response).agt('notValid'));
                }
            } else {
                $response = trim(fgets($stdin, 256));
                fclose($stdin);
