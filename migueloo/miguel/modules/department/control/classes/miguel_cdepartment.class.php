...
class miguel_CDepartment extends miguel_Controller
{	
	/**
	 * This is the constructor.
	 *
	 */
	function miguel_CDepartment()
	{	
		$this->miguel_Controller();
		$this->setModuleName('department');
		$this->setModelClass('miguel_MDepartment');
	...
	function processPetition() 
	{
            ...
            if ( $institution_id >= 0 ) { 
                ...
                $arr_categories = $this->obj_data->getDepartmentResume($institution_id, $faculty_id);
                if ( !$arr_categories[0]['id'] ) {
                    header('Location:' . Util::format_URLPath('area/index.php', 'institution_id=' . $institution_id . '&faculty_id=' . $faculty_id . '&department_id=0') );
                }
                
                $this->setViewVariable('arr_categories', $arr_categories );
                $this->setViewVariable('arr_courses', $this->obj_data->getCourse( $institution_id, $faculty_id ) );

                $this->addNavElement(Util::format_URLPath('department/index.php','institution_id=' . $institution_id . '&faculty_id=' . $faculty_id), agt('miguel_Departament') );
                $this->setCacheFile("miguel_VMain_Department_" . $this->getSessionElement("userinfo", "user_id"));
                $this->setMessage(agt('miguel_DepartmentList') );
                $this->setPageTitle( 'miguel_DepartmentList' );
